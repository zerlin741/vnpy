...
from .constant import Exchange, Interval, Offset, Direction


log_formatter = logging.Formatter('[%(asctime)s] %(message)s')


def update_ctaposdetails(pos_detail, trade):
    if trade.offset == Offset.OPEN:
        if trade.direction == Direction.LONG:
            # pos_detail.long_pos.price = (pos_detail.long_pos.price * pos_detail.long_pos.volume \
            #                              + trade.price * trade.volume) / (pos_detail.long_pos.volume + trade.volume)
            pos_detail.long_pos.volume += trade.volume
            pos_detail.long_pos.price_per.extend([trade.price]*trade.volume)
        elif trade.direction == Direction.SHORT:
            # pos_detail.short_pos.price = (pos_detail.short_pos.price * pos_detail.short_pos.volume \
            #                               + trade.price * trade.volume) / (pos_detail.short_pos.volume + trade.volume)
            pos_detail.short_pos.volume += trade.volume
            pos_detail.short_pos.price_per.extend([trade.price]*trade.volume)
    elif trade.offset == Offset.CLOSE:
        if trade.direction == Direction.LONG:
            # trade_pos = max(pos_detail.short_pos.volume, trade.volume)
            pos_detail.short_pos.volume -= trade.volume
            pos_detail.short_pos.price_per = pos_detail.short_pos.price_per[:-trade.volume]
        elif trade.direction == Direction.SHORT:
            # trade_pos = max(pos_detail.long_pos.volume, trade.volume)
            pos_detail.long_pos.volume -= trade.volume
            pos_detail.long_pos.price_per = pos_detail.long_pos.price_per[:-trade.volume]
    return pos_detail
...
class ArrayManager(object):
    ...
    def save_to_csv(self):
        import pandas as pd
        df = pd.DataFrame(self.bar_df, columns=['datetime', 'open', 'high', 'low', 'close'])
        df.to_csv('1m_kine.csv')
    ...
    def update_bar(self, bar: BarData) -> None:
        ...
        if not self.inited and self.count >= self.size:
            self.inited = True
        self.bar_df.append([bar.datetime, bar.open_price, bar.high_price, bar.low_price, bar.close_price])
